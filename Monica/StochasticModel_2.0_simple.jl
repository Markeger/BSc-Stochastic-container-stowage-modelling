...
function build_stochastic_model_2(data_CP, data_omega, data_ship, N_scenarios)
    ...
    # presence of 20' containers to load or unload? per scenario
    @variable(model, phi[1:N, 1:P, 1:L], Bin)
    ...
    @constraint(model, [i = 1:N, tau_20 = 1:T_20, t = TR_FU[pc]],
        sum(s_20[i, tau_20, t, l] for l in 1:L) <= omgea[1][transport_keys[t]][tau_20]
    )
    @constraint(model, [i = 1:N, tau_40 = 1:T_40, t = TR_FU[pc]],
        sum(s_40[i, tau_40, t, l] for l in 1:L) <= omgea[1][transport_keys[t]][T_20 + tau_40]
    )
    @constraint(model, [i = 1:N, tau_20 = 1:T_20, t = TR_FU[pc]],
        omgea[1][transport_keys[t]][tau_20] - sum(s_20[i, tau_20, t, l] for l in 1:L) == q_20[i, tau_20, t]
    )
    @constraint(model, [i = 1:N, tau_40 = 1:T_40, t = TR_FU[pc]],
        omgea[1][transport_keys[t]][T_20 + tau_40] - sum(s_40[i, tau_40, t, l] for l in 1:L) == q_40[i, tau_40, t]
    )

    # Lower containers unload later constraint per scenario (8)
    @constraint(model, [i = 1:N, p = 1:P, l in L_O],
        sum(x_20[tau_20, t, n] for t in intersect(TR_A[p], TR_DEP[pc]), tau_20 in 1:T_20, n in L_U if n > 0) +
        sum(x_40[tau_40, t, n] for t in intersect(TR_A[p], TR_DEP[pc]), tau_40 in 1:T_40, n in L_U if n > 0) +
        sum(s_20[i, tau_20, t, n] for t in intersect(TR_A[p], TR_FU[pc]), tau_20 in 1:T_20, n in L_U if n > 0) +
        sum(s_40[i, tau_40, t, n] for t in intersect(TR_A[p], TR_FU[pc]), tau_40 in 1:T_40, n in L_U if n > 0)
        <= M * delta[i, p, l]
    )
