...
function build_deterministic_model_simple(data, data_ship)
    ...
    TR = length(LD[1])  # Number of transports (sets of loading and discharge ports) #((P-1)*P/2)
    ...
    @constraint(DeterministicModel_Simple, [p = 1:P, l = 1:L],
        sum(x_20[tau_20, t, l] for tau_20 in reefers20, t in TR_ON[p]) + 2 * 
        sum(x_40[tau_40, t, l] for tau_40 in reefers40, t in TR_ON[p])
        <= C_R[l]
        )
    ...
    @constraint(DeterministicModel_Simple, [tau_20 = 1:T_20, t = 1:TR], 
        ...
        <= LD[1][transport_keys[t]][tau_20]          # Relaxed to not enclude all containers
        )
    @constraint(DeterministicModel_Simple, [tau_40 = 1:T_40, t = 1:TR], 
        sum(x_40[tau_40, t, l] for l in 1:L) 
        <= LD[1][transport_keys[t]][T_20 + tau_40]   # Relaxed to not enclude all containers
        )
    # Minimise non-loaded containers constraint
    @constraint(DeterministicModel_Simple, [tau_20 = 1:T_20, t = 1:TR],
        LD[1][transport_keys[t]][tau_20] -
        sum(x_20[tau_20, t, l] for l in 1:L)
        == z_20[tau_20, t]
        )
    @constraint(DeterministicModel_Simple, [tau_40 = 1:T_40, t = 1:TR],
        LD[1][transport_keys[t]][T_20 + tau_40] -
        sum(x_40[tau_40, t, l] for l in 1:L)
        == z_40[tau_40, t]
        )
    ...
    return DeterministicModel_Simple, x_20, x_40, z_20, z_40
end