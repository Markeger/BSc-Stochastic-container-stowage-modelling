...
struct ShipInstance
	...
	n_locations::Int64
	n_bins::Int64
	locations_over::Vector{Float64}
	locations_under::Vector{Float64}
	locations_over_in_bay::Vector{Vector{Float64}}
	...
	bay_bins::Vector{Vector{Float64}}
...
function read_ship_instance(filename::String)
	...
	# Try parsing as Int64, fallback to Float64 if error (for robustness)
	# Always parse as Float64 for locations_over and locations_under
	locations_over = parse.(Float64, split(readline(file)))
	locations_under = parse.(Float64, split(readline(file)))
	locations_over_in_bay = []
	for b in 1:n_bays
		vals = parse.(Float64, split(readline(file)))
		if lastindex(vals) > 1
			push!(locations_over_in_bay, vals[2:end])
		else
			push!(locations_over_in_bay, Float64[])
		end
	end
	...
	# location_weight_capacity may contain decimals, so parse as Float64
	...
	bay_bins = []
	for b in 1:n_bins
		push!(bay_bins, parse.(Float64, split(readline(file))))
	end
	# All bay-related values may contain decimals, so parse as Float64
